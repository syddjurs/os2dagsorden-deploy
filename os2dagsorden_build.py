#!/usr/bin/env python
"""
Build script for OS2Dagsorden.

Utilises Drush make to build a site or install profile based on OS2Dagsorden.
"""
...
def make_command(options, make_path):
    ...
    if options.developer:
        make_file = 'os2dagsorden.dev.make'
    else:
        make_file = 'os2dagsorden.make'

    # Set command based on mode.
    if options.mode == 'site':
        command = ['drush', 'make', '--contrib-destination=profiles/os2dagsorden', make_file, make_path]
...
def setup_profile(options, make_path):
    ...
    """
    if options.mode == 'site':
        path = os.path.join(make_path, 'profiles', 'os2dagsorden')
    else:
        path = make_path

    shutil.copy('os2dagsorden.profile', path)
    shutil.copy('os2dagsorden.info', path)
    shutil.copy('os2dagsorden.install', path)
